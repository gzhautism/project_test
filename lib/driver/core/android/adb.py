...
from lib.driver.core.android.constant import (DEFAULT_ADB_PATH, IP_PATTERN,
                                           SDK_VERISON_ANDROID7)
from lib.driver.core.error import (AdbError, AdbShellError, AutotestError,
                                   DeviceConnectionError)
from lib.driver.utils.compat import decode_path, raisefrom, proc_communicate_timeout, SUBPROCESS_FLAG
from lib.driver.utils.logger import logger
from lib.driver.utils.nbsp import NonBlockingStreamReader
from lib.driver.utils.retry import retries
from lib.driver.utils.snippet import get_std_encoding, reg_cleanup, split_cmd, make_file_executable
...
class ADB(object):
    ...
    def devices(self, state=None):
        """
        Perform `adb devices` command and return the _list of adb devices

        Args:
            state: optional parameter to filter devices in specific state

        Returns:
            _list od adb devices
    ...
    def get_forwards(self):
        """
        Perform `adb forward --_list`command
    ...
    def install_multiple_app(self, filepath, replace=False, install_options=None):
        """
            ...
            Args:
                filepath: full path to file to be installed on the device
                replace: force to replace existing application, default is False
                install_options:  _list of options
                    e.g.["-t",  # allow test packages
                        "-l",  # forward lock application,
                        "-s",  # install application on sdcard,
                        "-d",  # allow version code downgrade (debuggable packages only)
    ...
    def swipe(self, tuple_x0y0, tuple_x1y1, duration=500):
        ...
        Raises:
            AutotestError: if SDK version is not supported
        ...
        if version <= 15:
            raise AutotestError('swipe: API <= 15 not supported (version=%d)' % version)
        elif version <= 17:
    ...
    def get_top_activity(self):
        ...
        Raises:
            AutotestError: if top activity cannot be obtained
        ...
        if m:
            ...
            raise AutotestError("Can not get top activity, output:%s" % dat)
    ...
    def is_screenon(self):
        ...
        Raises:
            AutotestError: if screen state can't be detected
        ...
        raise AutotestError("Couldn't determine screen ON state")
    ...
    def is_locked(self):
        ...
        Raises:
            AutotestError: if lock screen can't be detected
        ...
        if not m:
            raise AutotestError("Couldn't determine screen lock state")
        return (m.group(1) == 'true')
    ...
    def list_app(self, third_only=False):
        """
        Perform `adb shell pm _list packages` to print all packages, optionally only
    ...
    def path_app(self, package):
        ...
        Raises:
            ...
            AutotestError: if package is not found on the device
        ...
        if 'package:' not in output:
            raise AutotestError('package not found, output:[%s]' % output)
    ...
    def check_app(self, package):
        ...
        Raises:
            AutotestError: if package is not found
        ...
        if match is None:
            raise AutotestError('package "{}" not found'.format(package))
        return True
    ...
    def start_app_timing(self, package, activity):
        ...
        if not re.search(r"Status:\s*ok", out):
            raise AutotestError("Starting App: %s/%s Failed!" % (package, activity))
