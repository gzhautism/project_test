...
import re
import six
from lib.driver.core.android.yosemite import Yosemite
from lib.driver.core.android.constant import YOSEMITE_PACKAGE
from lib.driver.core.error import AutotestError
from lib.driver.utils.logger import logger
from lib.driver.utils.nbsp import NonBlockingStreamReader
from lib.driver.utils.snippet import on_method_ready
LOGGING = logger
...
class Recorder(Yosemite):
    ...
    def start_recording(self, max_time=1800, bit_rate=None):
        ...
        if getattr(self, "recording_proc", None):
            raise AutotestError("recording_proc has already started")
    ...
    def stop_recording(self, output="screen.mp4", is_interrupted=False):
        ...
        Raises:
            AutotestError: if recording was not started before
        ...
        for line in p.stdout.readlines():
            ...
            if m:
                ...
                return True
        raise AutotestError("start_recording first")
