...
from lib.driver.utils.logwraper import Logwrap, AutotestLogger
...
class G(object):
    ...
    BASEDIR = []
    LOGGER = AutotestLogger(setting.path.cur_log_path+"\\log.txt")
    LOGGING = logger
    SCREEN = None
    DEVICE = None
    DEVICE_LIST = []
...
def log(arg, timestamp=None, desc="", snapshot=False):
    ...
    """
    from lib.driver.core.cv import try_log_screen
    if G.LOGGER:
        depth = 0
        if snapshot:
...
def import_device_cls(platform):
    ...
    if platform in G.CUSTOM_DEVICES:
        ...
        from lib.driver.core.android.android import Android as cls
    else:
