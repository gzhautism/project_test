...
from lib.driver.core.error import AdbShellError
...
from lib.poco.sdk.Attributor import Attributor
from lib.poco.sdk.interfaces.screen import ScreenInterface
from lib.poco.utils.hrpc.hierarchy import RemotePocoHierarchy
from lib.poco.utils.driver.input import DriverInput
from lib.poco.utils import six
from lib.poco.drivers.android.utils.installation import install, uninstall
...
class AndroidPocoAgent(PocoAgent):
    def __init__(self, endpoint, ime, use_driver_input=False):
...
class AndroidUiautomationPoco(Poco):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if AndroidUiautomationPoco.__instance is None:  # 只有第一次的时候，条件是None，所以第一次的时候才创建一个对象，后面的条件都不是false，所以直接返回第一次创建的对象
            obj = object.__new__(cls)  # 继承调用object的new方法产生一个对象
            AndroidUiautomationPoco.__instance = obj  # 将这个对象赋值给这个类的私有变量__instance

        return AndroidUiautomationPoco.__instance

    def __init__(self, device=None, using_proxy=True, force_restart=False, use_driver_input=False, **options):
        ...
        agent = AndroidPocoAgent(endpoint, self.ime, use_driver_input)
    ...
    def _is_running(self, package_name):
        ...
        :param package_name: package name(e.g., com.github.uiautomator)
                            or regular expression(e.g., poco\|driver\|uiautomator\|airbase)
        :return: pid or None
        """
    ...
    def on_pre_action(self, action, ui, args):
        if self.screenshot_each_action:
            # airteset log用
            from lib.driver.core.api import snapshot
            msg = repr(ui)
...
class AndroidUiautomationHelper(object):
    ...
    def get_instance(cls, device):
        ...
        simply pass the ``current device instance`` provided by ``driver`` to get the AndroidUiautomationPoco instance.
        If no such AndroidUiautomationPoco instance, a new instance will be created and stored. 

        Args:
            device (:py:obj:`driver.core.device.Device`): more details refer to ``driver doc``
