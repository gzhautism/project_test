...
class HierarchyInterface(object):
    ...
    def select(self, query, multiple):
        """
        Select UI element(s) matching the given query expression and return the _list of selected UI element(s)
        ...
        Returns:
            :obj:`_list` : _list of UI elements corresponding to the given query expression
        """
    ...
    def getAttr(self, nodes, name):
        ...
        Args:
            nodes: UI element or _list of UI elements, if there is a _list of UI elements provided, then only the \
